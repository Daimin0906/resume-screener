...
class CacheManager:
    ...
    def delete_prefix(self, prefix: str) -> int:
        """
        删除所有以指定前缀开头的缓存键（如人工反馈后失效对应查询的结果缓存）
        ...
        try:
            for key in list(self.cache.iterkeys()):
                if key.startswith(prefix):
                    ...
                    count += 1
            if count:
                logger.info(f"Cache invalidated {count} keys with prefix: {prefix}")
            return count
        except Exception as e:
            logger.error(f"Failed to delete cache keys with prefix {prefix}: {e}")
            return 0
