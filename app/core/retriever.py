...
from app.core.metadata_utils import deserialize_metadata, serialize_metadata
...
class Retriever:
    ...
    def update_resume_preclassification(self, resume_id: str, preclassification: Dict[str, Any]) -> None:
        """把预分类结果持久化到向量库元数据（服务重启后可恢复）。

        Args:
            resume_id (str): 简历ID
            preclassification (Dict[str, Any]): 预分类结果 {classification, reason, rule_version, ...}
        """
        try:
            self.vector_store_manager.update_metadata(
                collection_name="resumes",
                ids=[resume_id],
                metadatas=[serialize_metadata({"preclassification": preclassification})],
            )
        except Exception as e:
            logger.warning(f"Failed to persist preclassification for {resume_id}: {e}")
