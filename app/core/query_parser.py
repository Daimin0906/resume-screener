...
请提取以下字段：
- keywords: 关键词列表
- required_skills: 核心必需技能（硬性要求），最多5个。只收录明确的技术栈关键词
  （如 Python、FastAPI、RAG、Agent），不要收录职责/行为描述词
  （如"脚本编写""接口调用""数据处理""基础后端服务开发""能独立完成""有经验"等）
- preferred_skills: 优先/加分技能列表；除核心必需技能外，其他提到的技能都归入这里
