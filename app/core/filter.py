...
class HardFilter:
    ...
    def _filter_by_skills(self, resumes: List[Dict[str, Any]], required_skills: List[str]) -> List[Dict[str, Any]]:
        ...
        for resume in resumes:
            ...
            # 检查必需技能的命中比例：结构化字段优先，回退到简历全文
            # （LLM 常把技能写进正文而漏填 skills 字段）
            # 注意：完整 JD 常被解析出大量"必需"技能，全部命中不现实，
            # 因此按命中比例判定（默认 ≥70% 即通过），避免误杀合格候选人。
            fulltext = self._resume_text(resume)
            matched = sum(
                1 for skill in required_skills if self._matches(skill, skills, fulltext)
            )
            hit_ratio = matched / len(required_skills) if required_skills else 1.0

            if hit_ratio >= settings.REQUIRED_SKILL_HIT_RATIO:
                filtered_resumes.append(resume)
