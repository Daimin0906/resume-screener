...
配置默认岗位要求后，实现 Codeex 工作流第 4 步「固化与自动化运行」：

```
邮箱收到简历 → 自动抓取入库（每 30 分钟）→ 新简历解析完成
→ 自动按默认岗位要求跑完整筛选（无需人工点按钮）
→ 结果持久化到 data/auto_screen_results.json
→ 前端「自动筛选」面板展示：时间/简历数/三分类分布/候选人列表
```

- **默认岗位要求**：界面「自动筛选」面板填写保存（存 `data/default_query.txt`），随时可改
- **触发**：每次邮箱抓取后自动筛选所有"新简历"（已筛选过的不重复筛）；也可点「立即运行一次」
- **规则联动**：自动筛选用当前生效规则（人工纠正→总结规则后自动生效），结果带 `rules_version_used`
- **防重入**：筛选运行中不会叠加新任务；失败的批次自动留待下轮重试
- **相关 API**：`GET/PUT /auto-screen/query`、`POST /auto-screen/run`、`GET /auto-screen/results`、`GET /auto-screen/status`
